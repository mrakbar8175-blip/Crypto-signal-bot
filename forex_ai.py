...
def get_yahoo_forex_klines(pair, interval='4h', days=60):
    """Fetch forex data from yfinance. Pair must be like 'EURUSD=X'."""
    symbol = pair + "=X"
    end = datetime.now()
...
def get_buying_pressure(pair):
    ...
    df = df.tail(48)
    # yfinance may not provide volume for forex; fallback to price change accumulation
    if 'Volume' in df.columns and df['Volume'].sum() > 0:
        ...
        # Use close‑to‑close directional strength as proxy
...
def get_volatility_score(pair, current_price):
    ...
    if atr_pct < 0.2 or atr_pct > 2.0:   # forex sweet spot
        return -1, atr_err
    return 1, None
...
def dxy_trend_score():
    ...
    current = closes.iloc[-1]
    if current > ema50.iloc[-1]:
        return 2, None   # DXY up = bullish USD, bearish for EUR/USD etc.
    else:
        return -2, None
...
def generate_signal():
    ...
    for item in pairs_with_price:
        ...
        if atr / price > 0.02:   # 2% ATR cap for forex
            total_score = 0.0
